...
def main():
    parser = argparse.ArgumentParser(
        description="Detect salient audio events on dashcam pairs."
    )
    parser.add_argument(
        "--base-dir",
        type=str,
        required=True,
        help="Path to DCIM folder containing clips (same as src.events).",
    )
    parser.add_argument(
        "--index",
        type=int,
        default=0,
        help="Clip pair index to analyze (0-based, same as src.events).",
    )
    parser.add_argument(
        "--camera",
        choices=["road", "cabin", "both"],
        default="road",
        help="Which camera to analyze. You said audio is identical, "
             "so this is mostly for consistency / debugging.",
    )
    parser.add_argument(
        "--k",
        type=float,
        default=1.8,
        help="Threshold multiplier on salience score (higher = fewer events).",
    )
    parser.add_argument(
        "--pre-event-sec",
        type=float,
        default=1.5,
        help="Seconds of padding before peak time.",
    )
    parser.add_argument(
        "--post-event-sec",
        type=float,
        default=2.5,
        help="Seconds of padding after peak time.",
    )
    parser.add_argument(
        "--min-event-gap-sec",
        type=float,
        default=1.0,
        help="Minimum gap between events before they are merged.",
    )
    parser.add_argument(
        "--max-events",
        type=int,
        default=10,
        help="Maximum number of events to keep (by peak score).",
    )
    parser.add_argument(
        "--debug-npy-prefix",
        type=str,
        default=None,
        help="If set, save score & feature arrays as .npy files with this prefix.",
    )

    args = parser.parse_args()

    base = Path(args.base_dir)
    cfg = DashcamConfig(base_dir=base)
    ...
    if args.index >= len(pairs):
        raise IndexError(
            f"pair_index {args.index} out of range (found {len(pairs)})"
        )

    pair = pairs[args.index]
    print(f"Using pair #{args.index}:")
    ...
    if args.camera == "both":
        ...
        cameras_to_run = [args.camera]

    for cam in cameras_to_run:
        video_path = _select_video_from_pair(pair, cam)
        print(f"\n[audio] Running on {cam.upper()} camera ({video_path.name})")
        ...
        if args.debug_npy_prefix:
            ...
            np.save(f"{prefix}_score.npy", features["score"])
            np.save(f"{prefix}_rms.npy", features["rms"])
            np.save(f"{prefix}_flux.npy", features["flux"])
            np.save(f"{prefix}_low_ratio.npy", features["low_ratio"])
            np.save(f"{prefix}_mid_ratio.npy", features["mid_ratio"])
            np.save(f"{prefix}_high_ratio.npy", features["high_ratio"])
            np.save(f"{prefix}_centroid.npy", features["centroid"])
            np.save(f"{prefix}_frame_times.npy", features["frame_times"])
            print(f"  Saved debug features with prefix '{prefix}_*.npy'")
